...
def test_volume_contract_references_known_subjects_and_turns() -> None:
    volume = {"volumes": [{"id": "volume-1", "promise": "找到母带", "conflict": "证据被删", "climax": "公开母带", "closure": "真相公开", "turn_refs": ["turn-1"], "cast_ids": ["subject-lin"], "thread_ids": [], "length_hint": "short"}]}
...
def test_detail_requires_exact_frozen_chapter_refs_and_formal_cast() -> None:
    detail = {"chapters": [{"ref": "chapter-1", "volume_ref": "volume-1", "purpose": "取得母带", "pov": "subject-lin", "cast_ids": ["subject-lin"], "scenes": [{"place": "档案室", "objective": "取证", "conflict": "封锁", "turn": "发现签名", "result": "拿到副本"}], "handoff": "追查签名"}]}
