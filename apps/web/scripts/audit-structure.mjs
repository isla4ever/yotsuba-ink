...
for (const file of sourceFiles) {
  ...
  if (/from\s+["'][^"']*(?:\/components\/|\/views\/|\/context["']|\/types["'])/.test(source)) {
...
console.log(`Structure audit passed (${sourceFiles.length} TypeScript source files).`)
...
async function collectSourceFiles(root) {
  ...
  for (const entry of entries) {
    ...
    else if (/\.(?:ts|tsx)$/.test(entry.name) && !entry.name.endsWith(".test.ts") && !entry.name.endsWith(".test.tsx")) files.push(absolute)
  }
  return files
}
