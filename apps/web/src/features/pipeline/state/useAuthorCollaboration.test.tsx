...
import { useAuthorCollaboration, type AuthorCollaborationController } from './useAuthorCollaboration';
...
vi.mock('../services/authorCollaborationApi', () => ({
  acceptCollaborationPatch: mocks.acceptPatch,
...
describe('useAuthorCollaboration lifecycle', () => {
  beforeEach(() => {
    ...
    mocks.getThread.mockImplementation(async (_runId: string, threadId: string) => detail(
    ...
    mocks.createThread.mockImplementation(async (_runId: string, input: {
      source_ref?: string;
      unit_ref?: string;
    }) => thread('thread-created', input.unit_ref ?? 'artifact', input.source_ref ?? 'spine-v1'));
    mocks.createTurn.mockResolvedValue({});
    mocks.acceptPatch.mockResolvedValue({
      patch_id: 'patch-1',
      status: 'accepted',
      writeback_ref: 'spine-draft-2',
    });
  });

  it('exposes the accepted writeback revision so the stage draft can refresh', async () => {
    ...
    function Harness() {
      controller = useAuthorCollaboration({
        artifactText: JSON.stringify({ turns: [{ id: 'turn-1' }] }),
        enabled: true,
        runId: 'run-1',
        sourceRef: 'spine-v1',
        stageId: 'spine',
    ...
    await act(async () => {
      await requireController(controller).patchDecision({ patch_id: 'patch-1', thread_id: 'thread-a' } as never, 'accept');
      await settleEffects();
    });

    expect(requireController(controller).acceptedPatchRef).toBe('patch-1:spine-draft-2');
    act(() => root.unmount());
  });
  ...
  it('survives the StrictMode effect remount and resets the SSE cursor before switching threads', async () => {
    ...
    function Harness() {
      controller = useAuthorCollaboration({
        artifactText: JSON.stringify({ turns: [{ id: 'turn-1' }, { id: 'turn-2' }] }),
        enabled: true,
        runId: 'run-1',
        sourceRef: 'spine-v1',
        stageId: 'spine',
    ...
    expect((controller as AuthorCollaborationController | null)?.activeThreadId).toBe('thread-a');
    expect((controller as AuthorCollaborationController | null)?.detail?.thread.thread_id).toBe('thread-a');
    ...
    await act(async () => {
      (controller as AuthorCollaborationController | null)?.switchThread('thread-b');
      await settleEffects();
    });

    expect((controller as AuthorCollaborationController | null)?.detail?.thread.thread_id).toBe('thread-b');
  ...
  it('keeps the first receipt pending when a selection switches to another scope thread', async () => {
    ...
    await act(async () => {
      await requireController(controller).submit(appendMessage('请收紧第二个转折。'), selection());
      await settleEffects();
    });
  ...
  it('sends the selected context policy and Source Pack in the preview request', async () => {
    ...
    act(() => {
      requireController(controller).setContextOption('include_canon_wiki', false);
      requireController(controller).toggleKnowledgeSource('kb-1');
    });
    await act(async () => {
      await requireController(controller).submit(appendMessage('核对母带公开规则。'), null);
      await settleEffects();
    });
    ...
    expect(requireController(controller).pendingSubmission?.input.context_policy?.source_pack_refs).toEqual(['kb-1']);
  ...
  it('shows the lightweight reminder only before the second turn', async () => {
    ...
    vi.useFakeTimers();
    await act(async () => {
      await requireController(controller).submit(appendMessage('第二轮问题。'), null);
    });
    ...
    await act(async () => {
      await requireController(controller).submit(appendMessage('第三轮问题。'), null);
    });
    expect(requireController(controller).notice).toBe('');
    vi.useRealTimers();
  ...
  it('creates and reuses threads only for the current Artifact source and unit', async () => {
    mocks.listThreads.mockResolvedValue([
      thread('thread-old-source', 'turn-1', 'spine-committed-old'),
      thread('thread-wrong-unit', 'turn-2', 'spine-candidate-current'),
    ]);
    mocks.getThread.mockImplementation(async (_runId: string, threadId: string) => detail(
      threadId === 'thread-created'
        ? thread('thread-created', 'turn-1', 'spine-candidate-current')
        : thread('thread-old-source', 'turn-1', 'spine-committed-old'),
    ));
    const container = document.createElement('div');
    const root = createRoot(container);
    ...
    function Harness() {
      controller = useAuthorCollaboration({
        ...
        enabled: true,
        runId: 'run-1',
        sourceRef: 'spine-candidate-current',
        stageId: 'spine',
      });
      return null;
    }
    ...
    expect(mocks.createThread).toHaveBeenCalledWith('run-1', expect.objectContaining({
      source_ref: 'spine-candidate-current',
      unit_ref: 'turn-1',
    }));
    expect(requireController(controller).activeThreadId).toBe('thread-created');
    expect(requireController(controller).sourceRef).toBe('spine-candidate-current');
  ...
  it('switches the active collaboration thread with the workbench secondary navigation scope', async () => {
    ...
    function Harness({ unitRef }: { unitRef: string }) {
      controller = useAuthorCollaboration({
        activeUnit: { label: unitRef === 'turn-1' ? '转折一' : '转折二', unitRef },
        artifactText: JSON.stringify({ turns: [{ id: 'turn-1' }, { id: 'turn-2' }] }),
        enabled: true,
        runId: 'run-1',
        sourceRef: 'spine-v1',
    ...
    expect(requireController(controller).detail?.thread.scope.unit_ref).toBe('turn-2');
  ...
  it('keeps unsent composer drafts isolated when switching collaboration threads', async () => {
    ...
    act(() => requireController(controller).setComposerDraft('第一条线程尚未发送的推敲。'));
    expect(requireController(controller).composerDraft).toBe('第一条线程尚未发送的推敲。');
    ...
    act(() => requireController(controller).setComposerDraft('第二条线程的草稿。'));
    ...
    expect(requireController(controller).composerDraft).toBe('第一条线程尚未发送的推敲。');
    expect(requireController(controller).hasComposerDraft('thread-b')).toBe(true);
  ...
  it('restores the exact first-turn request when the author abandons context confirmation', async () => {
    ...
    expect(requireController(controller).composerDraft).toBe('');
    expect(requireController(controller).pendingSubmission?.input.message).toBe(request);
  ...
  it('keeps the same idempotent submission available when turn creation fails', async () => {
    ...
    await act(async () => {
      await requireController(controller).submit(appendMessage('保持同一请求重试。'), null);
      await settleEffects();
    });
    ...
    expect(mocks.createTurn).toHaveBeenCalledWith('run-1', 'thread-a', expect.objectContaining({
...
function latestSubscription(threadId: string) {
  const records = mocks.observe.mock.calls
    .map((call) => ({ threadId: call[1] as string, options: call[2] as {
      after: number;
...
function requireController(controller: AuthorCollaborationController | null): AuthorCollaborationController {
  if (!controller) throw new Error('Author collaboration controller is not ready');
  return controller;
}
...
function thread(threadId: string, unitRef: string, sourceRef = 'spine-v1'): CollaborationThread {
  return {
    thread_id: threadId,
    run_id: 'run-1',
    project_id: 'project-1',
    ...
    title: `脊柱 · ${unitRef}`,
    provider_binding: {} as CollaborationThread['provider_binding'],
...
function streamEvent(sequence: number, threadId: string): CollaborationStreamEvent {
  return {
    sequence,
