...
describe("createSelectionAnchor", () => {
  it("converts browser UTF-16 offsets to Python-compatible Unicode offsets", async () => {
    ...
    expect(anchor.selection_start).toBe(
      Array.from(fieldValue.slice(0, selectionStart)).length,
    )
    expect(anchor.selection_end).toBe(
      anchor.selection_start + Array.from(selectedText).length,
    )
  ...
  it("keeps an anchored stage selection while the collaboration composer receives focus", async () => {
    ...
    const stageField = container.querySelector<HTMLTextAreaElement>(
    ...
    expect(
      container.querySelector('[data-testid="selection-preview"]')?.textContent,
    ).toBe("主角公开证据")
...
async function waitForText(container: HTMLElement, expected: string) {
  for (let attempt = 0; attempt < 40; attempt += 1) {
    if (
      container.querySelector('[data-testid="selection-preview"]')
