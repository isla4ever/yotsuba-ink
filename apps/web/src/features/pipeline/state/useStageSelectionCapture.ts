...
export function useStageSelectionCapture({ enabled, rootRef, sourceRef, stageId }: Options) {
  ...
  useEffect(() => {
    ...
    const capture = (event: Event) => {
      ...
      if (!isSelectionTarget(target) || !rootRef.current?.contains(target)) return;
    ...
    const invalidate = (event: Event) => {
      const target = event.target;
      if (!isSelectionTarget(target) || !rootRef.current?.contains(target)) return;
      if (selection?.field_path === target.dataset.collaborationFieldPath) clearSelection();
    };
  ...
  }, [clearSelection, enabled, rootRef, selection?.field_path, sourceRef, stageId]);
...
export async function createSelectionAnchor({
  ...
  return {
    anchor_id: globalThis.crypto?.randomUUID?.() ?? `selection-${Date.now()}-${codePointStart}`,
    stage_id: stageId,
...
function selectionPreview(value: string) {
  ...
  return Array.from(compact).length > 80 ? `${Array.from(compact).slice(0, 80).join('')}...` : compact;
}

function isSelectionTarget(target: EventTarget | null): target is SelectionTarget {
