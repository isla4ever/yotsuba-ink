...
export function useAuthorCollaboration({
  activeUnit,
  artifactText,
  enabled,
  knowledgeDocuments,
  ...
  const [contextPolicy, setContextPolicy] =
    useState<CollaborationContextPolicy | null>(null)
  ...
  const [pendingSubmission, setPendingSubmission] =
    useState<PendingCollaborationSubmission | null>(null)
  const [latestReceipt, setLatestReceipt] =
    useState<CollaborationContextReceipt | null>(null)
  ...
  const knowledgeDocumentIds = useMemo(
    () =>
      new Set(
        knowledgeDocumentIdKey ? knowledgeDocumentIdKey.split("\u0000") : [],
      ),
  ...
  const refreshDetail = useCallback(
    async (threadId = activeThreadId, signal?: AbortSignal) => {
      ...
      setThreads((current) =>
        current.map((thread) =>
          thread.thread_id === record.thread.thread_id ? record.thread : thread,
        ),
      )
      return record
    },
      ...
      .then(async ([records, envelope]) => {
        ...
        if (envelope) {
          ...
          setContextPolicy({
            ...
            source_pack_refs:
              envelope.settings.context_policy.source_pack_refs.filter((ref) =>
                knowledgeDocumentIds.has(ref),
              ),
          })
        }
        const reusable = records.find((thread) =>
          collaborationThreadMatchesScope(
            thread,
            stageId,
            sourceRef,
        ...
        const thread =
          reusable ??
          (await createCollaborationThread(runId, {
            stage_id: stageId,
  ...
  useEffect(() => {
    setContextPolicy((current) =>
      current
        ? {
            ...current,
            source_pack_refs: current.source_pack_refs.filter((ref) =>
              knowledgeDocumentIds.has(ref),
            ),
          }
        : current,
  ...
  const ensureSelectionThread = useCallback(
    async (selection: SelectionAnchor | null) => {
  ...
  const dispatchSubmission = useCallback(
    async (submission: PendingCollaborationSubmission) => {
      ...
      try {
        ...
        await Promise.all([
          refreshDetail(submission.threadId),
          refreshThreads(),
        ])
      } catch (cause) {
        if (
          cause instanceof AuthorCollaborationApiError &&
          cause.code === "context_reconfirmation_required"
        ) {
          const receipt = await previewCollaborationContext(
            runId,
            submission.threadId,
            submission.input,
          )
  ...
  const submit = useCallback(
    async (message: AppendMessage, selection: SelectionAnchor | null) => {
      ...
      try {
        ...
        if (submissionThreadId !== draftOwnerThreadId)
          clearComposerDraft(submissionThreadId)
        const input: CollaborationContextPreviewInput = {
          client_turn_id:
            globalThis.crypto?.randomUUID?.() ?? `client-turn-${Date.now()}`,
          mode,
          message: content,
          context_policy: contextPolicy ?? settings?.context_policy,
        ...
        setLatestReceipt(receipt)
        const currentDetail =
          thread.thread_id === detail?.thread.thread_id
            ? detail
            : await getCollaborationThread(runId, thread.thread_id)
        if (currentDetail.thread.turn_count === 0) {
          setPendingSubmission(submission)
        } else {
          if (currentDetail.thread.turn_count === 1) {
            setNotice(
              `本轮将继续采用 ${receipt.sources.length} 项已确认上下文，约 ${receipt.token_estimate.toLocaleString()} tokens。`,
            )
            window.clearTimeout(noticeTimerRef.current)
    ...
    (docId: string) => {
      ...
      setContextPolicy((current) => {
        ...
        return {
          ...current,
          include_knowledge:
            selectedRefs.length > 0 || current.include_knowledge,
  ...
  const renameThread = useCallback(
    async (threadId: string, title: string) => {
      ...
      setThreads((current) =>
        current.map((thread) =>
          thread.thread_id === threadId ? record : thread,
        ),
      )
      if (detail?.thread.thread_id === threadId)
        setDetail((current) =>
          current ? { ...current, thread: record } : current,
        )
    },
  ...
  const archiveThread = useCallback(
    async (threadId: string) => {
      ...
      try {
        ...
        const next = records?.find((thread) =>
          collaborationThreadMatchesScope(
            thread,
            stageId,
            sourceRef,
  ...
  const rejectPatch = useCallback(
    async (patch: ArtifactPatchCandidate) => {
      setActionPending(true)
      setError("")
      try {
  ...
  const dismissPendingSubmission = useCallback(() => {
    if (pendingSubmission) {
      restoreComposerDraft(
        ...
        pendingSubmission.input.message,
      )
    }
    setPendingSubmission(null)
  ...
  return {
    ...
    confirmPendingSubmission: () =>
      pendingSubmission
        ? dispatchSubmission(pendingSubmission)
        : Promise.resolve(),
    composerDraft,
    contextPolicy,
    ...
    sourceRef:
      ...
      collaborationThreadMatchesScope(
        ...
        stageId,
        sourceRef,
        activeThread.scope.unit_ref,
      )
        ? activeThread.scope.artifact_ref
        : sourceRef,
    submit,
    switchThread,
    threads,
