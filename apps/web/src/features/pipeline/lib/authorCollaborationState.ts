...
export function collaborationThreadMatchesScope(
  ...
  return (
    ...
    (!sourceRef || thread.scope.artifact_ref === sourceRef) &&
...
export function appendMessageText(message: AppendMessage) {
  return message.content
    .filter(
      (part): part is Extract<typeof message.content[number], {
        type: "text"
      }> => part.type === "text",
    )
    .map((part) => part.text)
...
export function collaborationErrorMessage(error: unknown) {
  if (error instanceof AuthorCollaborationApiError) {
    const labels: Record<string, string> = {
      author_collaboration_unavailable: "作者协作仅在当前创作运行中可用。",
      ...
      patch_writeback_unavailable:
        "当前正式版本需要通过版本修订流程写回，不能原地覆盖。",
