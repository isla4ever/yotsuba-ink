...
export async function verifyExportBlob(
  ...
  if (expectedValue && !expected) throw new Error('交付 Receipt 的 SHA-256 无效，已阻止下载。');
  if (responseValue && !response) throw new Error('交付响应的 SHA-256 无效，已阻止下载。');
  if (expectation.requireResponseSha256 && !response) throw new Error('交付响应缺少 SHA-256，已阻止下载。');
  if (expectation.expectedSizeBytes != null && blob.size !== expectation.expectedSizeBytes) {
  ...
  if (!globalThis.crypto?.subtle) throw new Error('当前浏览器不支持交付文件完整性校验。');
  const digest = await globalThis.crypto.subtle.digest('SHA-256', await blob.arrayBuffer());
  const actual = Array.from(new Uint8Array(digest), (byte) => byte.toString(16).padStart(2, '0')).join('');
  if (actual !== target) throw new Error('交付文件 SHA-256 校验失败，已阻止下载。');
  return actual;
}
