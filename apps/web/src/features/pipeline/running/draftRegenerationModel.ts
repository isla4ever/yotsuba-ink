...
const STAGE_SUGGESTIONS: Partial<Record<StageType, readonly [string, string, string]>> = {
  ...
  detail: ['重分配事实增量：避免信息在同一章堆积。', '强化章末钩子：让每章自然进入下一章。', '提高 Wiki 清晰度：事实、伏笔、人物状态分开。'],
  info: ['强化悬疑钩子：让开篇问题更尖锐，增强追读冲动。', '增强人物关系：让主角、盟友和隐瞒者之间的张力更清楚。', '降低设定复杂度：减少解释负担，把设定转成行动线索。'],
...
export function regenerationSuggestionsForStage(stage?: Pick<WorkflowStage, 'type'>) {
  return [...(stage ? STAGE_SUGGESTIONS[stage.type] : STAGE_SUGGESTIONS.info) ?? DEFAULT_SUGGESTIONS];
}
