...
export function briefStageLoadingVisible(activeRunId: string, events: RunEvent[]) {
  if (!activeRunId) return false;
  const terminal = events.some((event) => (
    terminalTypes.has(event.type)
    && (!event.stage_id || event.stage_id === 'brief')
  ));
  return !terminal;
}

export function briefStageLoadingCopy(events: RunEvent[]) {
  const latest = events.find((event) => (
    referenceTypes.has(event.type)
    || (event.stage_id === 'brief' && (event.type === 'evidence.proposed' || event.type === 'node.started'))
...
export function briefStageLoadingMilestones(events: RunEvent[]) {
  const runStarted = events.some((event) => event.type === 'run.started');
  const generationStarted = events.some((event) => event.type === 'node.started' && event.stage_id === 'brief');
