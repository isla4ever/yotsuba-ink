...
export function CollaborationThread({
  ...
  const runtime = useExternalStoreRuntime<CollaborationMessage>({
    ...
    isSendDisabled:
      ...
      Boolean(collaboration.pendingSubmission) ||
      (collaboration.mode === "revise" && (!selection || !canRevise)),
  ...
  const UserMessage = () => (
    <CollaborationMessageView collaboration={collaboration} role="user" />
  )
  const AssistantMessage = () => (
    <CollaborationMessageView collaboration={collaboration} role="assistant" />
  )
  return (
        ...
        <ThreadPrimitive.Viewport className="collaboration-thread-viewport">
          <ThreadPrimitive.Empty>
            <div className="collaboration-empty-state">
              ...
              <p>
                讨论因果、人物动机或语言质地；需要改稿时先选择当前字段中的原文。
              </p>
            </div>
          </ThreadPrimitive.Empty>
          ...
          {collaboration.isStreaming ? (
            <div className="collaboration-generating-row">
              <GeneratingIndicator />
            </div>
          ) : null}
...
function CollaborationMessageView({
  collaboration,
  role,
}: {
  collaboration: AuthorCollaborationController
  ...
  const message = collaboration.detail?.messages.find(
    (item) => item.message_id === messageId,
  )
  if (!message) return null
  const turn = collaboration.detail?.turns.find(
  ...
  return (
    <MessagePrimitive.Root className={`collaboration-message ${role}`}>
      <header>
        ...
        <small>
          {modeLabel(message.mode)} · {formatTime(message.created_at)}
        </small>
      </header>
      {role === "user" && turn?.selection_anchor ? (
        <div className="collaboration-message-selection">
          <Quote size={12} />
          <span>已引用 {turn.selection_anchor.selected_char_count} 字</span>
          <code>{turn.selection_anchor.field_path}</code>
        </div>
      ) : null}
      <MessagePrimitive.Parts />
      {role === "user" && turn && !turn.assistant_message_ref ? (
        <CollaborationTurnOutcome turn={turn} />
      ) : null}
      {message.plan ? (
        <section className="collaboration-plan">
          ...
          {message.plan.risks.length ? (
            <p>风险：{message.plan.risks.join("；")}</p>
          ) : null}
        </section>
      ) : null}
      {patch ? (
        <PatchCandidateView
          ...
          onReject={() => {
            void collaboration.rejectPatch(patch)
          }}
          patch={patch}
        />
      ) : null}
      {role === "assistant" ? (
        <footer className="collaboration-message-actions">
          ...
          <button onClick={() => collaboration.setMode("plan")} type="button">
            <Check size={13} />
            转为方案
          </button>
        </footer>
...
function convertCollaborationMessage(
