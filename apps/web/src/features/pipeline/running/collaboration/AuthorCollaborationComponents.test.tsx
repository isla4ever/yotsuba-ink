...
import type { ArtifactPatchCandidate, CollaborationContextPolicy, CollaborationContextReceipt, CollaborationTurn } from "../../contracts/authorCollaboration"
...
describe('author collaboration decision surfaces', () => {
  it('distinguishes cancelled, failed, and unknown-outcome recovery states', () => {
    const cancelled = renderToStaticMarkup(<CollaborationTurnOutcome turn={turn('cancelled')} />);
    const failed = renderToStaticMarkup(<CollaborationTurnOutcome turn={turn('failed')} />);
    const unknown = renderToStaticMarkup(<CollaborationTurnOutcome turn={turn('failed', 'provider_outcome_unknown')} />);
  ...
  it('renders a source-bound before/after decision instead of applying silently', () => {
    ...
    expect(proposed).toContain('应用到当前稿');
  ...
  it('keeps the pending request visible in the first full receipt and allows explicit abandonment', async () => {
    ...
    expect(document.body.textContent).toContain('请把主角的公开代价改得更具体。');
    expect(document.body.textContent).toContain('当前转折');
    const abandon = Array.from(document.body.querySelectorAll<HTMLButtonElement>('button'))
  ...
  it('lets the author select distinct context systems and project knowledge sources', async () => {
    ...
    const trigger = document.body.querySelector<HTMLButtonElement>('[aria-label="选择本轮上下文"]');
    await act(async () => {
      trigger?.click();
      await Promise.resolve();
    });
    ...
    const knowledge = Array.from(document.body.querySelectorAll<HTMLButtonElement>('button'))
...
function patch(status: ArtifactPatchCandidate['status']): ArtifactPatchCandidate {
  return {
    patch_id: 'patch-1',
    thread_id: 'thread-1',
    turn_id: 'turn-1',
...
function turn(status: CollaborationTurn['status'], code = ''): CollaborationTurn {
  return {
    turn_id: 'turn-1',
