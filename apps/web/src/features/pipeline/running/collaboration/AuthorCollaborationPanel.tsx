...
export function AuthorCollaborationPanel({
  canRevise,
  collaboration,
  knowledgeDocuments,
  onClearSelection,
  ...
  const saveTitle = () => {
    ...
    if (
      trimmed &&
      collaboration.activeThread &&
      trimmed !== collaboration.activeThread.title
    ) {
      void collaboration
        .renameThread(collaboration.activeThread.thread_id, trimmed)
        .catch(() => undefined)
    }
    setTitleEditing(false)
  }
  const archiveBlocked =
    collaboration.actionPending ||
    ...
    >
      <header className="collaboration-panel-header">
        <div className="collaboration-panel-identity">
          <span>
            <Sparkles size={15} />
            {collaborationStageLabels[stageId]}
          </span>
          {titleEditing ? (
            <input
              aria-label="对话标题"
            ...
            <button
              ...
              type="button"
            >
              <strong>{collaboration.activeThread?.title ?? "作者协作"}</strong>
              <Pencil size={12} />
            </button>
          )}
          <small>
            {collaboration.composerDraft.trim()
              ? `未发送草稿已保留 · ${shortProviderLabel(collaboration.detail)}`
              : shortProviderLabel(collaboration.detail)}
          </small>
        </div>
        <div className="collaboration-panel-actions">
          <button
            aria-label="查看上下文回执"
            disabled={!collaboration.latestReceipt}
          ...
          <button
            aria-label="归档当前对话"
            disabled={!collaboration.activeThread || archiveBlocked}
            onClick={() => {
              if (collaboration.activeThread)
                void collaboration.archiveThread(
                  collaboration.activeThread.thread_id,
                )
            }}
            title={
              collaboration.composerDraft.trim()
      ...
      {collaboration.notice ? (
        <div className="collaboration-notice" role="status">
          {collaboration.notice}
        </div>
      ) : null}
      {collaboration.error ? (
      ...
      {collaboration.activeThread?.status === "archived" ? (
        <div className="collaboration-readonly-banner">
          这段历史对话已归档，可阅读但不能继续发送。
        </div>
      ) : null}
      <CollaborationThread
        canRevise={canRevise}
      ...
      <CollaborationHistoryDrawer
        ...
        onArchive={(threadId) => {
          void collaboration.archiveThread(threadId)
        }}
        onClose={() => setHistoryOpen(false)}
        onDelete={(threadId) => {
          void collaboration.deleteThread(threadId)
        }}
        onNew={() => {
          void collaboration.newThread()
          setHistoryOpen(false)
        }}
        onSwitch={(threadId) => {
          collaboration.switchThread(threadId)
          setHistoryOpen(false)
        }}
        open={historyOpen}
      ...
      <ContextReceiptDialog
        ...
        onConfirm={() => {
          void collaboration
            ...
            .then(() => setReceiptOpen(false))
        }}
        onOpenChange={setReceiptOpen}
        open={receiptOpen}
        pendingMessage={collaboration.pendingSubmission?.input.message}
        receipt={
          collaboration.pendingSubmission?.receipt ??
          collaboration.latestReceipt
        }
      />
    </aside>
  )
