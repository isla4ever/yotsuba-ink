    ...
    <ComposerPrimitive.Root className="collaboration-composer">
      {selection ? (
        <div className="collaboration-selection-chip">
          ...
          <small>
            {selection.selected_char_count} 字 · {selection.field_path}
          </small>
          <button
            aria-label="移除选区"
      ...
      <div className="collaboration-composer-tools">
        <div
          aria-label="协作模式"
          className="collaboration-mode-switch"
          role="group"
        >
          {(["discuss", "plan", "revise"] as CollaborationMode[]).map(
            (value) => (
              <button
                ...
                disabled={
                  disabled || (value === "revise" && (!canRevise || !selection))
                }
                key={value}
                onClick={() => onModeChange(value)}
                title={
                  value === "revise" && !canRevise
                    ? "正式版本需走修订分支，当前不可原地改稿"
                    : value === "revise" && !selection
                      ? "先选择一段合法字段文字"
                      : undefined
                }
                type="button"
              >
                {collaborationModeLabels[value]}
              </button>
            ),
          )}
        </div>
        <div className="collaboration-context-tools">
          ...
          <span>
            {latestReceipt
              ? `约 ${latestReceipt.token_estimate.toLocaleString()} tokens`
              : "发送前预检上下文"}
          </span>
        </div>
        {isStreaming ? (
          <ComposerPrimitive.Cancel
            aria-label="停止生成"
            className="collaboration-send stop"
            title="停止生成"
          >
            <StopCircle size={17} />
          </ComposerPrimitive.Cancel>
          ...
          <ComposerPrimitive.Send
            aria-label="发送"
            className="collaboration-send"
            title="发送（Command/Ctrl + Enter）"
          >
            <Send size={17} />
          </ComposerPrimitive.Send>
        )}
