  ...
  return (
    <aside aria-label="协作历史" className="collaboration-history-drawer">
      <header>
        <div>
          <span>对话历史</span>
          <strong>{threads.length} 个线程</strong>
        </div>
        <button aria-label="关闭历史" onClick={onClose} type="button">
          <X size={16} />
        </button>
      </header>
      <button
        className="collaboration-new-thread"
        disabled={disabled}
        onClick={onNew}
        type="button"
      >
      ...
      <div className="collaboration-history-list">
        {groupThreads(threads).map(([stageId, records]) => (
          <section key={stageId}>
            ...
            {records.map((thread) => (
              ...
              >
                <button
                  className="collaboration-history-main"
                  onClick={() => onSwitch(thread.thread_id)}
                  type="button"
                >
                  <strong>{thread.title}</strong>
                  <span>
                    {thread.provider_execution.model_id} ·{" "}
                    {formatTime(thread.updated_at)}
                  </span>
                  {thread.has_unapplied_patch ? (
                    <small>有一份改稿等待决定</small>
                  ) : hasDraft(thread.thread_id) ? (
                    <small>有未发送草稿</small>
                  ) : null}
                </button>
        ...
        {!threads.length ? (
          <p className="collaboration-history-empty">还没有对话记录。</p>
        ) : null}
      </div>
    </aside>
  )
...
function groupThreads(threads: CollaborationThread[]) {
  ...
  threads.forEach((thread) =>
    groups.set(thread.stage_id, [
      ...(groups.get(thread.stage_id) ?? []),
      thread,
    ]),
  )
