...
type ContextOption = "include_author_preferences" | "include_craft_mechanisms" | "include_knowledge" | "include_canon_wiki" | "include_foreshadow"
        ...
        >
          ...
          {policy ? (
            <div className="collaboration-context-option-list">
              <ContextOptionRow
                checked={policy.include_canon_wiki}
                label="相关 Canon / Wiki"
                onChange={(value) =>
                  onOptionChange("include_canon_wiki", value)
                }
              />
              <ContextOptionRow
                checked={policy.include_foreshadow}
                label="当前范围伏笔"
                onChange={(value) =>
                  onOptionChange("include_foreshadow", value)
                }
              />
              <ContextOptionRow
                checked={policy.include_author_preferences}
                label="作者偏好"
                onChange={(value) =>
                  onOptionChange("include_author_preferences", value)
                }
              />
              <ContextOptionRow
                checked={policy.include_craft_mechanisms}
                label="写作机制"
                onChange={(value) =>
                  onOptionChange("include_craft_mechanisms", value)
                }
              />
              <ContextOptionRow
                checked={policy.include_knowledge}
                label="本书知识库"
                onChange={(value) => onOptionChange("include_knowledge", value)}
              />
            </div>
          ) : null}
          <section className="collaboration-source-pack-list">
            <header>
              ...
              <button onClick={onOpenKnowledgeManager} type="button">
                <Settings2 size={13} />
                管理
              </button>
            </header>
            <div>
              {knowledgeDocuments.length ? (
                knowledgeDocuments.map((document) => {
                  const selected =
                    policy?.source_pack_refs.includes(document.doc_id) ?? false
