...
export type SpineMilestone = 'inciting' | 'commitment' | 'midpoint_reversal' | 'crisis' | 'climax' | 'aftermath';

export type StorySpineArtifact = {
  turns: Array<{ id: string; cause: string; change: string; progress_type: 'information' | 'relationship' | 'external' | 'internal'; milestones: SpineMilestone[] }>;
  ending: string;
...
type Schema = { type: 'string'; values?: readonly string[]; allowEmpty?: boolean } | { type: 'number'; integer?: boolean; min?: number; nullable?: boolean } | { type: 'array'; items: Schema; min?: number; max?: number } | { type: 'object'; fields: Record<string, Schema> };
...
const array = (items: Schema, min = 0, max?: number): Schema => ({ type: 'array', items, min, max });
const object = (fields: Record<string, Schema>): Schema => ({ type: 'object', fields });

const BRIEF_SCHEMA = object({ title: string(), premise: string(), promise: string(), world_rules: array(string(), 1), theme: string(), ending_promise: string(), voice: string(), length_envelope: object({ word_target_soft: number({ min: 1, integer: true }) }) });
const SPINE_SCHEMA = object({ turns: array(object({ id: string(), cause: string(), change: string(), progress_type: string({ values: ['information', 'relationship', 'external', 'internal'] }), milestones: array(string({ values: ['inciting', 'commitment', 'midpoint_reversal', 'crisis', 'climax', 'aftermath'] }), 0, 6) }), 1), ending: string(), open_questions: array(string()), progress_types: array(string({ values: ['information', 'relationship', 'external', 'internal'] }), 1) });
const VOLUMES_SCHEMA = object({ volumes: array(object({ id: string(), title: string(), promise: string(), conflict: string(), climax: string(), climax_turn_ref: string(), closure: string(), turn_refs: array(string(), 1), cast_ids: array(string(), 1), length_hint: string({ values: ['short', 'medium', 'long'] }) }), 1) });
const DETAIL_SCHEMA = object({ chapters: array(object({ ref: string(), volume_ref: string(), title: string(), target_characters: number({ min: 1, integer: true, nullable: true }), turn_refs: array(string(), 1), purpose: string(), pov: string(), cast_ids: array(string(), 1), scenes: array(object({ place: string(), objective: string(), conflict: string(), turn: string(), result: string() }), 1, 12), handoff: string() }), 1) });
...
export const parseSpineArtifact = (source: string) => parseArtifact<StorySpineArtifact>(source, SPINE_SCHEMA, (value, errors) => {
  validateOrderedIdentity('turns', 'turn', 'id')(value, errors);
  if (!isRecord(value) || !Array.isArray(value.turns) || !Array.isArray(value.progress_types)) return;
  const projected = [...new Set(value.turns.map((turn) => isRecord(turn) ? turn.progress_type : '').filter(Boolean))];
  if (JSON.stringify(projected) !== JSON.stringify(value.progress_types)) errors.push('推进维度必须由逐转折推进类型自动投影');
  const milestones = value.turns.flatMap((turn) => isRecord(turn) && Array.isArray(turn.milestones) ? turn.milestones : []);
  ['inciting', 'commitment', 'midpoint_reversal', 'crisis', 'climax', 'aftermath'].forEach((milestone) => {
    if (milestones.filter((item) => item === milestone).length !== 1) errors.push(`结构里程碑 ${milestone} 必须且只能出现一次`);
  });
});
...
export const parseDetailArtifact = (source: string) => parseArtifact<DetailArtifactVnext>(source, DETAIL_SCHEMA, (value, errors) => {
  ...
  value.chapters.forEach((item) => {
    ...
    if (!Array.isArray(item.turn_refs) || item.turn_refs.length === 0) errors.push(`${String(item.ref)} 必须绑定至少一个剧情转折`);
...
function validateSchema(value: unknown, schema: Schema, path: string, errors: string[]) {
  ...
  if (schema.type === 'array') { if (!Array.isArray(value)) { errors.push(`${path} 必须是数组`); return; } if (schema.min && value.length < schema.min) errors.push(`${path} 至少包含 ${schema.min} 项`); if (schema.max && value.length > schema.max) errors.push(`${path} 最多包含 ${schema.max} 项`); value.forEach((item, index) => validateSchema(item, schema.items, `${path}[${index}]`, errors)); return; }
