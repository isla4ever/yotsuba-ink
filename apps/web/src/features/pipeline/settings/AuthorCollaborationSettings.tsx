...
export function AuthorCollaborationSettings() {
  ...
  const [status, setStatus] =
    useState<"loading" | "ready" | "saving" | "saved" | "failed">("loading")
  ...
  return (
    <div className="collaboration-settings-panel">
      <header className="collaboration-settings-intro">
        <div>
          <span>AUTHOR COLLABORATION · MULTI-TURN</span>
          <h1>作者协作</h1>
          <p>
            ...
            >
              {unavailableBinding && (
                <option value={selectedCapabilityKey}>
                  当前绑定不可用 · {draft.default_provider_profile_id} ·{" "}
                  {draft.default_model}
                </option>
              )}
      ...
      <section className="collaboration-settings-block">
        ...
        <div className="collaboration-policy-list">
          <PolicyToggle
            checked={draft.context_policy.include_author_preferences}
            label="作者偏好"
            onChange={(value) =>
              patchPolicy({ include_author_preferences: value })
            }
          />
          <PolicyToggle
            checked={draft.context_policy.include_craft_mechanisms}
            label="写作机制包"
            onChange={(value) =>
              patchPolicy({ include_craft_mechanisms: value })
            }
          />
          <PolicyToggle
            checked={draft.context_policy.include_canon_wiki}
            label="相关 Canon / Wiki"
            onChange={(value) => patchPolicy({ include_canon_wiki: value })}
          />
          <PolicyToggle
            checked={draft.context_policy.include_foreshadow}
            label="当前范围伏笔"
            onChange={(value) => patchPolicy({ include_foreshadow: value })}
          />
          <PolicyToggle
            checked={draft.context_policy.include_knowledge}
            label="知识库 Source Pack"
            onChange={(value) => patchPolicy({ include_knowledge: value })}
          />
        </div>
        <div className="collaboration-settings-grid compact">
          <label>
            ...
            <input
              ...
              min={4000}
              onChange={(event) =>
                patchPolicy({ max_input_chars: Number(event.target.value) })
              }
              step={1000}
              type="number"
              value={draft.context_policy.max_input_chars}
            />
          </label>
          <label>
            <span>最多历史轮次</span>
            <input
              ...
              min={0}
              onChange={(event) =>
                patchPolicy({ max_history_turns: Number(event.target.value) })
              }
              type="number"
              value={draft.context_policy.max_history_turns}
            />
          </label>
        </div>
      </section>
        ...
        <div className="collaboration-settings-wide-fields">
          <label>
            ...
            <textarea
              className="input"
              onChange={(event) =>
                patchPolicy({ author_preferences: event.target.value })
              }
              placeholder="例如：克制叙述、动作可验证、避免总结式对白。"
              rows={5}
              value={draft.context_policy.author_preferences}
            />
          </label>
          <label>
            ...
            <textarea
              ...
              onChange={(event) =>
                patchPolicy({
                  craft_mechanisms: splitLines(event.target.value).slice(0, 12),
                })
              }
              placeholder={"场景-续篇\n因果压力测试\n人物语言护栏"}
              rows={5}
              value={draft.context_policy.craft_mechanisms.join("\n")}
            />
          </label>
        </div>
        <label className="collaboration-retention-field">
          ...
          <input
            ...
            onChange={(event) =>
              setDraft({
                ...draft,
                history_retention_days: Number(event.target.value),
              })
            }
            type="number"
        ...
        <div>
          {envelope.capabilities.map((item) => (
            <article data-ready={item.ready} key={capabilityKey(item)}>
              ...
              <Capability
                label="结构化改稿"
                value={item.supports_structured_patch}
              />
              <small>
      ...
      <footer className="collaboration-settings-savebar">
        <span>
          固定安全规则：改稿需确认、source binding、无静默 Provider
          fallback、对话无 Canon/Wiki 直接写权。
        </span>
        <button
...
export function CollaborationRunBinding({
  run,
}: {
  run: Phase32RunEnvelope | null
}) {
  const available = Boolean(run)
  const bindings = collaborationBindings(run)
  return (
    <section className="collaboration-run-binding">
      <header>
        <div>
          <span>作者协作</span>
          <strong>{available ? "当前模式已启用" : "进入创作运行后可用"}</strong>
        </div>
        <ShieldCheck size={16} />
      </header>
      <p>
        新线程采用全局作者协作默认并在创建时冻结
        Provider；已经存在的线程不会随设置变化而静默换模。
      </p>
      <div>
        {bindings.map((binding) => (
          <article key={binding.stageId}>
            <span>{binding.label}</span>
            <strong>{binding.identity}</strong>
          </article>
        ))}
      </div>
    </section>
...
function collaborationBindings(
  run: Phase32RunEnvelope | null,
): CollaborationBinding[] {
  if (!run) return []
  const definition = run.definition as Phase32RunDefinition
  const readModel = run.read_model as Phase32RunReadModel
  const byStage = new Map(
    definition.provider_bindings_by_stage.map((binding) => [
      binding.stage_id,
      binding.binding.payload,
    ]),
  )
  return readModel.stage_manifest
    .filter((stage) => stage.collaboration_enabled)
    .map((stage) => {
      const execution = byStage.get(stage.stage_id)?.execution
      const values =
...
function capabilityKey(item: CapabilityKeyInput) {
