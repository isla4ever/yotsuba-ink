...
import type { ProviderProfile, WorkflowDefinition, WorkflowStage } from '../types/workflow';
import { modelNameForUi, providerKindForUi, providerNameForUi } from '../utils/display';
                ...
                <section className="settings-section">
                  ...
                  {globalText ? (
                    <ModelOptionInput
                      ...
                      onChange={(model) => updateProvider({ ...globalText, default_model: model })}
                    />
                  ) : null}
                  {globalImage ? (
                    <ModelOptionInput
                      ...
                      onAddOption={addModelOption}
                      onChange={(model) => updateProvider({ ...globalImage, default_model: model })}
                    />
                  ) : null}
                    ...
                    {workflow.provider_profiles.map((provider) => (
                      <article className="provider-editor" key={provider.id}>
                        ...
                        <ModelOptionInput
                          ...
                          onAddOption={addModelOption}
                          onChange={(model) => updateProvider({ ...provider, default_model: model })}
                        />
                      </article>
                    ))}
                  </div>
                      ...
                      return (
                        <article key={stage.id}>
                          ...
                          <ModelOptionInput
                            ...
                            onAddOption={addModelOption}
                            onChange={(model) => updateStage({ ...stage, model_settings: { ...stage.model_settings, model } })}
                          />
                        </article>
                      );
                    })}
