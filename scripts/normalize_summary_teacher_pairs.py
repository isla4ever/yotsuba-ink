"""
Normalize summary teacher pairs to fixed content schema.
...
from deploy.openai_compat_server import _normalize_summary_text
...
def _normalize_line(obj: Dict) -> Dict:
    ...
    normalized = _normalize_summary_text(raw, req.get("messages", []))
    normalized = re.sub(r"\n[ \t]{8,}", "\n", normalized).strip()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Normalize summary teacher pair responses")
