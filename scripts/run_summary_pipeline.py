...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run summary data+training pipeline")
    ...
    parser.add_argument("--run-tag", default="summary1000")
    ...
    req_pool = data_dir / f"summary_request_pool_{args.target_count}.jsonl"
    teacher_raw = data_dir / f"summary_teacher_pairs_deepseek_{args.target_count}.jsonl"
    teacher_norm = data_dir / f"summary_teacher_pairs_deepseek_{args.target_count}_norm_rtx3080.jsonl"
    sft_out = data_dir / f"summary_sft_deepseek_{args.target_count}_hq_rtx3080.jsonl"
    sft_report = logs_dir / f"summary_hq_correction_report_{args.target_count}_rtx3080.json"
    train_report = logs_dir / f"train_summary_hqcorr_resilient_{args.run_tag}_report.json"
    train_output_root = outputs_dir / f"summary_hqcorr_resilient_{args.run_tag}"
    run_log = logs_dir / f"pipeline_summary_{args.run_tag}.log"
    ...
    if not base_inputs:
        raise SystemExit("No input structured jsonl found for summary request pool.")
    ...
    if force or not (resume and _is_nonempty_file(req_pool)):
        cmd = [
            api_py,
            "scripts/build_summary_request_pool.py",
            "--input",
    ...
    if force or not (resume and _is_nonempty_file(teacher_norm)):
        cmd = [
            api_py,
            "scripts/normalize_summary_teacher_pairs.py",
            "--input",
            str(teacher_raw),
            "--output",
            str(teacher_norm),
    ...
    if force or not (resume and _is_nonempty_file(sft_out)):
        cmd = [
            api_py,
            "scripts/build_summary_hq_correction_sft.py",
            "--input",
            str(teacher_norm),
            "--output",
            str(sft_out),
    ...
    cmd = [
        train_py,
        "scripts/run_summary_hq_resilient_train.py",
        "--model-path",
        args.model_path,
        "--data",
        str(sft_out),
