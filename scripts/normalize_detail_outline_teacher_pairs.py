"""
Normalize detail-outline teacher pairs to fixed content schema.
...
def _normalize_line(obj: Dict) -> Dict:
    ...
    normalized = _normalize_detail_outline_text(raw, req.get("messages", []))
    normalized = re.sub(r"\n[ \t]{8,}", "\n", normalized).strip()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Normalize detail-outline teacher pair responses")
