"""
End-to-end outline pipeline runner:
1) Build outline request pool
2) Generate teacher pairs via DeepSeek
3) Normalize teacher responses
4) Build HQ outline SFT dataset
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run outline data+training pipeline")
    ...
    parser.add_argument("--run-tag", default="outline1000")
    ...
    req_pool = data_dir / f"outline_request_pool_{args.target_count}.jsonl"
    teacher_raw = data_dir / f"outline_teacher_pairs_deepseek_{args.target_count}.jsonl"
    teacher_norm = data_dir / f"outline_teacher_pairs_deepseek_{args.target_count}_norm.jsonl"
    sft_out = data_dir / f"outline_sft_deepseek_{args.target_count}_hq.jsonl"
    sft_report = logs_dir / f"outline_hq_correction_report_{args.target_count}.json"
    train_report = logs_dir / f"train_outline_hqcorr_resilient_{args.run_tag}_report.json"
    train_output_root = outputs_dir / f"outline_hqcorr_resilient_{args.run_tag}"
    run_log = logs_dir / f"pipeline_outline_{args.run_tag}.log"

    base_inputs: List[str] = [
        "data/novel_task_structured_from_zh_corpus_1500.jsonl",
        "data/summary_sft_deepseek_1000_hq_correction_v5_mix1000.jsonl",
        "data/info_recommend_sft_deepseek_1000_norm_final.jsonl",
    ]
    ...
    if force or not (resume and _is_nonempty_file(req_pool)):
        cmd = [
            api_py,
            "scripts/build_outline_request_pool.py",
            "--input",
            *base_inputs,
            "--output",
            str(req_pool),
    ...
    if force or not (resume and _is_nonempty_file(teacher_norm)):
        cmd = [
            api_py,
            "scripts/normalize_outline_teacher_pairs.py",
            "--input",
            str(teacher_raw),
            "--output",
            str(teacher_norm),
    ...
    if force or not (resume and _is_nonempty_file(sft_out)):
        cmd = [
            api_py,
            "scripts/build_outline_hq_correction_sft.py",
            "--input",
            str(teacher_norm),
            "--output",
            str(sft_out),
    ...
    cmd = [
        train_py,
        "scripts/run_outline_resilient_train.py",
        "--model-path",
        args.model_path,
        "--data",
        str(sft_out),
