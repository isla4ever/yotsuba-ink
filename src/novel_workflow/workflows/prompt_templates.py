...
CAST_PROMPT = """你是人物编排编辑。基于 Story Brief、Story Spine、role_demand_proposals 和预分配的 subject_refs，只返回 CharacterDossierBatch JSON。subjects 每项只写 name、kind、function、background、conflict_history、present_stakes、temperament、speech_style、drive、change、debut、limits、demand_refs，不返回 subject id 或 relations；运行时按 demand_key 将档案绑定到 subject_refs，再由独立窄调用生成关系。每个 demand 恰好由一个档案覆盖，不得创造 demand。每个档案必须是独立的具名主体：name 互不相同，同一个人不得占用多个主体位；name 必须是真实可读的人名，不能写“某人的母亲”“前任档案员”“委员会代表”等关系或职务标签。kind 只允许 protagonist、major、functional、npc、historical_record 五个值：反派、对手写 major 或 functional，不存在 antagonist 等其他值。
模式配对是封闭合同：protagonist、opposition、relationship、functional 必须使用 subject_mode=actor；只有 narrative_role=historical_record 才能使用 subject_mode=historical_record。历史主体只能写记录、证词、遗物、声音、遗产或缺席如何影响当代判断，不能写其在当下行动、说话、调查、选择、施压、互动或拥有 POV；不得把历史主体放进 present actor demand，也不得把当代人物伪装成历史记录。subject_refs.subject_mode 和 narrative_role 是代码冻结的硬合同。历史主体的 background 写其生前已成立的身份和经历，conflict_history 写其记录与核心冲突的既定联系，present_stakes 写其遗产或证词当下可能失去的可信度、归属或影响；drive 写生前诉求，change 写其证词、遗产或缺席在叙事中的意义变化；不得虚构其当下行动、当下说话或 POV。
...
DETAIL_PROMPT = """你是章节施工图编辑。当前输入只包含一个最小 volume_contract 执行投影、该卷对应的 volume_spine_turns、scale_projection、selected_dossiers、world_rule_projection 和可选 previous_segment_handoff。volume_contract 不是完整分卷 Artifact：非末分段只有 id/title，末分段才包含 closing_state。world_rule_projection 是 Brief 原始规则的只读结构化投影，rule_id、source_text、future_offset_hours、fixed_time 与 recurrence 均不可改写。只返回这个叙事单元的 DetailSegmentArtifact；每章只写 title、purpose、pov、cast_ids、scenes、handoff，不返回 chapter ref、volume ref 或 turn_refs，运行时按调用顺序和冻结槽位绑定。
...
人物与程序硬约束：selected_dossiers[].limits 每一条都必须遵守；historical_record_ids 是禁止进入 cast_ids 的精确集合，present_actor_ids 是当代 cast_ids 的唯一候选集合。historical_record 不得在当下行动、说话、拥有 POV 或直接互动，只能作为记录、回忆或缺席被提及。机构保持机构形态，除非当前 turn 明确要求，不得新增具名负责人、秘密协助者、阴谋、闯入、干扰监控、藏匿证据或其他捷径。cast_ids 只记录本章现场实际行动的主体，被文书、回忆或对话提到不算出场。返回前逐章核对 cast_ids 与两个结构化集合。
世界规则硬约束：每章的时间、职业行为、身份变化和物理状态必须遵守 world_rule_projection。recurrence=periodic 或 loop 只允许规则所声明的周期或回环形式，不允许相邻章节复写相同结果；每次回环仍须带来新的行动、知识、关系或风险状态。
...
篇幅纪律：scale.scene_length 是当前场景的滚动字符合同，按 non_whitespace_characters（去除空白）统计。场景不要求机械等长，可以随戏剧负载不同而变化，但片段必须靠近 target_characters 并落在 min_characters 与 max_characters 之间，使全部场景组装后仍能命中冻结章节合同；篇幅不足时只在 scene.execution 六个微节拍内展开尝试与受阻、空间动作、已有职业行为、在场人物潜台词、POV 判断和即时后果，不新增事件或可验证事实；篇幅超标时删除重复心理复述、解释性总结和无效过场，不删掉施工图要求的转折。严禁为了组织段落或描述流程新增任何序数、层级、编号标签或量化事实（例如“第一层”“第二层”“第3步”“第一条”“第二条”“第三条”）；若施工图没有提供该词或数值，就不要写。需要写一组记录、日志或物件时，直接写“日志”“记录”“一条记录”等，不要自行编号。
...
COVER_PROMPT = """你是小说封面编辑。基于 accepted_story_metadata 和 visual_decisions 返回唯一 CoverBrief，只写 concept、image_prompt、palette、negative_constraints。negative_constraints 最多返回 24 项，建议只保留 8-12 项最重要的排除项；不得为了凑数拆分同义词或重复表达。不要读取正文全文，不要返回资产 id、URL、候选状态或导出信息。"""
