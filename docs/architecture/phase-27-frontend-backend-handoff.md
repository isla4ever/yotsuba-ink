...
> 状态：后端 Phase 27 vNext 合同与 LangGraph fake-provider 离线门已通过；本文件是交给下一位 UI Agent 的交接合同，不是对当前 UI WIP 的继承要求。真实 Provider、浏览器矩阵、长篇文学质量和 GitHub 发布仍未完成。
>
...
- `uv run pytest -q`：`278 passed, 9 warnings`。
- `python3 -m compileall -q src tests`：通过。
- `git diff --check`：通过。
- 既有前端基线记录过 Vitest、Vite build、CSS audit 和 CSS split 通过；由于 UI 由新 Agent 重做，必须在新 UI 完成后重新执行，不能把旧截图或旧测试结果当作新 UI 证据。

这些结果只证明合同、fake Provider、Graph、存储和 API 的离线行为。它们不证明 DeepSeek 真实 JSON 稳定性、真实图片 Provider、SSE 网络断线、8-12 章单卷质量、投稿质量或作者冷读。真实验收必须从全新的 Run 开始，不能恢复历史 Run `phase26-deepseek-*`。
...
| `volumes` | `VolumeArchitectureArtifact` | `volumes[] {id,promise,conflict,climax,closure,turn_refs,cast_ids,thread_ids,length_hint}`；`volume-*` 和 turn 覆盖由代码校验 | 每卷是否是一个可独立成立的故事，边界、高潮、闭合和卷间承接 | `ArtifactStore.volumes` | 卷承诺/冲突/高潮/闭合、连续 turn refs、相关主体/线程、短中长提示 | 当前卷合同、相关 cast/thread；不带固定 chapter window |
...
- 必须可见：角色档案、关系压力、demand 来源、debut 窗口、kind/tier、孤立/过载等诊断。
...
- 主任务：审阅每个自然闭合的卷合同：`promise/conflict/climax/closure`，以及 `turn_refs/cast_ids/thread_ids/length_hint`。
...
- 章节边界：由叙事边界、POV 交接、地点/时间断裂、独立线程和 token ceiling 决定；不是固定 8 章切片。
