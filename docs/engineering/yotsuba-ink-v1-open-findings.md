...
- Character Bible 后续按“核心身份、叙事功能、人物弧、写作护栏”四组重新评估字段粒度，优先合并语义重叠的长文本字段；迁移前保持当前 Artifact 合同和稳定 ID 不变，不在 UI 层拼装第二套 JSON。
- 角色数量保持叙事需求驱动，不设置固定少量上限。长篇模式需要验证 30-60 名主体时的解析、名册虚拟化、星图降级和上下文裁剪，历史主体继续与当下可行动角色分开计数。
